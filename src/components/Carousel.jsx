...
class Carousel extends Component {
	...
	render() {
		let { list, sizePadding } = this.props
		let settings = {
			customPaging: () => {
				return (
		...
		return <div ref={(el) => {
			this.carousel = el
		}} className="Carousel" style={{ maxWidth: this.props.deviceWidth }} onTouchMove={e => e.preventDefault()}>
			<Slider {...settings}>
				{list.map((cover, key) => {
					return <div key={key} onTouchMove={e => e.preventDefault()}>
						<div
							...
							{!cover.preview ? <ThumbImage url={cover.image.url} sizePadding={sizePadding} />
								: <div className="Carousel__video"
									style={{
										background: `#FFFFFF url(${cover.preview}) no-repeat center center/contain `,
